...
//			}
            ...
            case Zap.State.ON_GROUND:
                ...
                if( groundUnderFeet2 == null || distToGround > 0.1f)
                {
                    zap.setState(Zap.State.IN_AIR);
                    setAction(Action.JUMP);
                    wantGetUp = false;
                }
                else if (groundUnderFeet2)
                {
...
//				}
	...
	int tryJumpFromRope(bool forceJumpOff = false){
		...
		if (Input.GetKeyDown (zap.keyJump) || forceJumpOff) {
			...
			if (Input.GetKey (zap.keyLeft) && !forceJumpOff ) { //skacze w lewo
				zap.turnLeft ();
			...
			} else if (Input.GetKey (zap.keyRight) && !forceJumpOff ) { //skacze w prawo
				zap.turnRight ();
