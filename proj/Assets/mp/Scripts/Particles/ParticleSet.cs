...
public class ParticleSet : ScriptableObject
{
    ...
    public ParticleData GetParticleData(string ParticleTag)
    {
        int numberOfDatas = ParticleDatas.Length;
        for (int i = 0; i < numberOfDatas; ++i)
        {
            ParticleData pd = ParticleDatas[i];
            if (pd.ParticleTag == ParticleTag) return pd;
        }
        return null;
    }
